...
const EMPTY = { platform: '', icon: '', description: '', profileUrl: '' };
...
export default function AdminAchievementsPage() {
    ...
    const load = () => { setLoading(true); fetch('/api/achievements').then(r => r.json()).then(d => { setItems(Array.isArray(d) ? d : []); setLoading(false); }).catch(() => setLoading(false)); };
    useEffect(load, []);

    const save = async () => {
        setSaving(true);
        const isNew = modal === 'new';
        const res = await fetch(isNew ? '/api/achievements' : `/api/achievements/${modal.id}`, { method: isNew ? 'POST' : 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(form) });
        setSaving(false);
    ...
    const del = async (id) => {
        ...
        if ((await fetch(`/api/achievements/${id}`, { method: 'DELETE' })).ok) { toast.success('Deleted!'); load(); }
    };
        ...
        <div style={{ padding: '2rem 2.5rem' }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '2rem' }}>
                <div>
                    ...
                    <h1 style={{ fontSize: '1.75rem', fontWeight: 800, color: 'var(--text-primary)' }}>Achievements</h1>
                </div>
            ...
            {loading ? <div style={{ display: 'flex', justifyContent: 'center', padding: '4rem' }}><Loader2 size={28} style={{ color: 'var(--accent-cyan)', animation: 'spin 1s linear infinite' }} /></div>
                : items.length === 0 ? <div className="card-base" style={{ padding: '3rem', textAlign: 'center', color: 'var(--text-muted)' }}>No achievements yet.</div>
                        ...
                        {items.map(item => (
                            <div key={item.id} className="card-base" style={{ padding: '1.25rem', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '1rem', flexWrap: 'wrap' }}>
                                <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
                                    ...
                                    <div>
                                        <p style={{ fontWeight: 700, color: 'var(--text-primary)' }}>{item.platform}</p>
                                        <p style={{ color: 'var(--text-secondary)', fontSize: '0.82rem' }}>{item.description}</p>
                                    </div>
                                </div>
                    ...
                    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={() => setModal(null)}
                        ...
                        <motion.div initial={{ scale: 0.95 }} animate={{ scale: 1 }} onClick={e => e.stopPropagation()}
                            ...
                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem' }}>
                                <h2 style={{ fontWeight: 700, color: 'var(--text-primary)' }}>{modal === 'new' ? 'Add Achievement' : 'Edit Achievement'}</h2>
                                <button onClick={() => setModal(null)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--text-muted)' }}><X size={20} /></button>
                            </div>
                            <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                                {[['platform', 'Platform *'], ['icon', 'Icon (emoji)'], ['description', 'Description *'], ['profileUrl', 'Profile URL']].map(([key, label]) => (
                                    <div key={key}>
