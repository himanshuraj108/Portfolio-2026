...
const EMPTY = { name: '', role: '', company: '', message: '', avatar: '', featured: false };
...
export default function AdminTestimonialsPage() {
    ...
    const load = () => { setLoading(true); fetch('/api/testimonials').then(r => r.json()).then(d => { setItems(Array.isArray(d) ? d : []); setLoading(false); }).catch(() => setLoading(false)); };
    useEffect(load, []);

    const save = async () => {
        setSaving(true);
        const isNew = modal === 'new';
        const res = await fetch(isNew ? '/api/testimonials' : `/api/testimonials/${modal.id}`, { method: isNew ? 'POST' : 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(form) });
        setSaving(false);
    ...
    const del = async (id) => {
        ...
        if ((await fetch(`/api/testimonials/${id}`, { method: 'DELETE' })).ok) { toast.success('Deleted!'); load(); }
    };
        ...
        <div style={{ padding: '2rem 2.5rem' }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '2rem' }}>
                <div>
                    ...
                    <h1 style={{ fontSize: '1.75rem', fontWeight: 800, color: 'var(--text-primary)' }}>Testimonials</h1>
                </div>
            ...
            {loading ? <div style={{ display: 'flex', justifyContent: 'center', padding: '4rem' }}><Loader2 size={28} style={{ color: 'var(--accent-cyan)', animation: 'spin 1s linear infinite' }} /></div>
                : items.length === 0 ? <div className="card-base" style={{ padding: '3rem', textAlign: 'center', color: 'var(--text-muted)' }}>No testimonials yet.</div>
                    : <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(300px, 1fr))', gap: '1.25rem' }}>
                        {items.map(item => (
                            <div key={item.id} className="card-base" style={{ padding: '1.25rem' }}>
                                {item.featured && <span style={{ display: 'inline-flex', alignItems: 'center', gap: '0.25rem', padding: '0.2rem 0.5rem', background: 'rgba(245,158,11,0.12)', borderRadius: '5px', fontSize: '0.7rem', color: '#f59e0b', fontWeight: 600, marginBottom: '0.75rem' }}><Star size={10} fill="currentColor" />Featured</span>}
                                <p style={{ color: 'var(--text-secondary)', fontSize: '0.82rem', fontStyle: 'italic', lineHeight: 1.6, marginBottom: '0.75rem' }}>"{item.message?.slice(0, 120)}…"</p>
                                <p style={{ fontWeight: 700, color: 'var(--text-primary)', fontSize: '0.875rem' }}>{item.name}</p>
                                <p style={{ color: 'var(--text-muted)', fontSize: '0.78rem', marginBottom: '1rem' }}>{item.role}{item.company ? ` at ${item.company}` : ''}</p>
                    ...
                    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={() => setModal(null)}
                        ...
                        <motion.div initial={{ scale: 0.95 }} animate={{ scale: 1 }} onClick={e => e.stopPropagation()}
                            ...
                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem' }}>
                                <h2 style={{ fontWeight: 700, color: 'var(--text-primary)' }}>{modal === 'new' ? 'Add Testimonial' : 'Edit Testimonial'}</h2>
                            ...
                            <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                                {[['name', 'Name *'], ['role', 'Role'], ['company', 'Company'], ['avatar', 'Avatar URL'], ['message', 'Message *']].map(([key, label]) => (
                                    <div key={key}>
                                        <label style={{ display: 'block', fontSize: '0.78rem', fontWeight: 600, color: 'var(--text-secondary)', marginBottom: '0.4rem' }}>{label}</label>
                                        {key === 'message' ? <textarea value={form[key] || ''} onChange={e => setForm(p => ({ ...p, [key]: e.target.value }))} rows={4} className="form-input" style={{ resize: 'vertical' }} /> : <input value={form[key] || ''} onChange={e => setForm(p => ({ ...p, [key]: e.target.value }))} className="form-input" />}
                                    </div>
                                ))}
                                <label style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', cursor: 'pointer', fontSize: '0.875rem', color: 'var(--text-secondary)' }}>
                                    <input type="checkbox" checked={form.featured || false} onChange={e => setForm(p => ({ ...p, featured: e.target.checked }))} /> Featured
                                </label>
                            </div>
