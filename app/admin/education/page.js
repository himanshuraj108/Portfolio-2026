...
const EMPTY = { institution: '', degree: '', field: '', score: '', scoreType: 'CGPA', startYear: '', endYear: '', location: '', order: 0 };
...
export default function AdminEducationPage() {
    ...
    const load = () => { setLoading(true); fetch('/api/education').then(r => r.json()).then(d => { setItems(Array.isArray(d) ? d : []); setLoading(false); }).catch(() => setLoading(false)); };
    useEffect(load, []);

    const save = async () => {
        setSaving(true);
        const isNew = modal === 'new';
        const res = await fetch(isNew ? '/api/education' : `/api/education/${modal.id}`, { method: isNew ? 'POST' : 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ ...form, order: Number(form.order) || 0 }) });
        setSaving(false);
    ...
    const del = async (id) => {
        ...
        if ((await fetch(`/api/education/${id}`, { method: 'DELETE' })).ok) { toast.success('Deleted!'); load(); }
    };

    const fields = [['institution', 'Institution *'], ['degree', 'Degree *'], ['field', 'Field of Study'], ['score', 'Score'], ['scoreType', 'Score Type (CGPA/Percentage)'], ['startYear', 'Start Year'], ['endYear', 'End Year'], ['location', 'Location'], ['order', 'Order']];
        ...
        <div style={{ padding: '2rem 2.5rem' }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '2rem' }}>
                <div>
                    ...
                    <h1 style={{ fontSize: '1.75rem', fontWeight: 800, color: 'var(--text-primary)' }}>Education</h1>
                </div>
            ...
            {loading ? <div style={{ display: 'flex', justifyContent: 'center', padding: '4rem' }}><Loader2 size={28} style={{ color: 'var(--accent-cyan)', animation: 'spin 1s linear infinite' }} /></div>
                : items.length === 0 ? <div className="card-base" style={{ padding: '3rem', textAlign: 'center', color: 'var(--text-muted)' }}>No education entries yet.</div>
                    : <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                        {items.map(item => (
                            <div key={item.id} className="card-base" style={{ padding: '1.25rem', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '1rem', flexWrap: 'wrap' }}>
                                <div>
                                    <p style={{ fontWeight: 700, color: 'var(--text-primary)' }}>{item.institution}</p>
                                    <p style={{ color: 'var(--text-secondary)', fontSize: '0.875rem' }}>{item.degree} in {item.field}</p>
                                    <p style={{ color: 'var(--text-muted)', fontSize: '0.8rem' }}>{item.startYear} – {item.endYear} · {item.score} {item.scoreType} · {item.location}</p>
                                </div>
                                <div style={{ display: 'flex', gap: '0.5rem' }}>
                                    <button onClick={() => { setForm(item); setModal(item); }} style={{ display: 'flex', alignItems: 'center', gap: '0.35rem', padding: '0.4rem 0.8rem', borderRadius: '8px', background: 'var(--bg-tertiary)', border: '1px solid var(--border)', color: 'var(--text-secondary)', fontSize: '0.78rem', cursor: 'pointer' }}><Pencil size={12} />Edit</button>
                ...
                {modal && (
                    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={() => setModal(null)}
                        ...
                        <motion.div initial={{ scale: 0.95 }} animate={{ scale: 1 }} onClick={e => e.stopPropagation()}
                            className="card-base" style={{ width: '100%', maxWidth: 500, padding: '2rem', maxHeight: '90vh', overflowY: 'auto' }}>
                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem' }}>
                                <h2 style={{ fontWeight: 700, color: 'var(--text-primary)' }}>{modal === 'new' ? 'Add Education' : 'Edit Education'}</h2>
